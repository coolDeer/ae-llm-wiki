...
- 已读 `CLAUDE.md` 了解 wiki schema 和约定
...
2. **检查前置条件** —
   ...
   - 如果没有，提示用户先跑 `/fetch-reports` + ingest
3. **读取 mental map** —
...
- **Schema 引用**：`CLAUDE.md` / `AGENTS.md` 的工作流章节应在 ingest 之后提及"建议运行 daily-review"

## 相关文件

- `scripts/fetch_reports.py` — 上游数据拉取
- `skills/fetch-reports/SKILL.md` — 拉取 skill
- `.claude/commands/daily-review.md` — Slash command 入口
