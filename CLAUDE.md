...
> 本文件供 Claude Code 使用，内容与 `AGENTS.md` 保持同步。
> 若调整 wiki 规则、目录结构或工作流，需同时更新 `CLAUDE.md` 与 `AGENTS.md`。
...
| Schema | `CLAUDE.md` / `AGENTS.md` | 人 + LLM 共同维护 | 定义 wiki 运作规则，两份文件应保持一致 |
...
11. **建议运行 `/daily-summarize`** — daily-review 完成后，运行 daily-summarize skill 把 epistemic 复盘转换为 PM operational 简报，9 章节（执行摘要 / 市场快照 / 组合影响 / 新建仓 / 减仓 / 风险预警 / 催化剂日历 / 研究任务 / 路演要点），输出到 `wiki/output/daily-summarize-{date}.md`。这是 operational 决策环节，详见 `skills/daily-summarize/SKILL.md`。完整日循环：`fetch-reports → ingest → daily-review → daily-summarize`
